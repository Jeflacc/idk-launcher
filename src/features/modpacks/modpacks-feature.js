...
state.modpacks = state.modpacks.map(mp => ({ mods: [], resourcepacks: [], shaders: [], ...mp }));
  ...
  try {
    ...
    if (result.success && result.profiles.length > 0) {
      ...
      diskProfiles.forEach(p => {
        console.log(`[Modpacks] Disk profile: ${p.name} (${p.id}) ΓÇö mods:${p.diskMods?.length || 0} rp:${p.diskResourcepacks?.length || 0} sh:${p.diskShaders?.length || 0}`);
      });
      ...
      const mergeFiles = (diskFiles, storedFiles) => {
        // storedFiles might be an object or non-array ΓÇö normalize it
      ...
      // Build lookup maps ΓÇö by ID and by name (for legacy matching)
      ...
      // Build a new state.modpacks array entirely from disk ΓÇö disk is the source of truth
      ...
      const rebuildMsg = 'After rebuild: ' + state.modpacks.map(m => `${m.name}: mods=${m.mods?.length} rp=${m.resourcepacks?.length} sh=${m.shaders?.length}`).join(' | ');
      ...
      // The ID may have changed (legacy fix) ΓÇö try to find by old ID first, then keep first
...
function setImportProgress(status, percent) {
  ...
  if (launchStatus) launchStatus.innerText = status;
  if (launchFill && Number.isFinite(percent)) launchFill.style.width = `${Math.max(0, Math.min(100, percent))}%`;
}
...
function mpRenderInstalledList(type) {
  ...
  items.forEach((item, index) => {
    ...
    el.innerHTML = `
      ...
      <button class="remove-mod-btn" title="Remove">Γ£ò</button>`;
...
document.querySelectorAll('.mp-tab').forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    document.getElementById('btn-browse-mods').style.display = tab.dataset.tab === 'mods' ? 'block' : 'none';
    document.getElementById('btn-browse-rp').style.display = tab.dataset.tab === 'resourcepacks' ? 'block' : 'none';
    document.getElementById('btn-browse-shaders').style.display = tab.dataset.tab === 'shaders' ? 'block' : 'none';
  });
});
...
document.getElementById('btn-new-modpack').addEventListener('click', () => {
  ...
  if (sel.options.length === 0 && state.allVersions.length > 0) {
    state.allVersions.filter(v => v.type === 'release').slice(0, 30).forEach(v => {
...
document.getElementById('btn-close-mp-settings').addEventListener('click', () => {
  document.getElementById('mp-settings-modal').classList.remove('active');
});

document.getElementById('btn-cancel-mp-settings').addEventListener('click', () => {
  document.getElementById('mp-settings-modal').classList.remove('active');
});
...
document.getElementById('btn-export-modpack').addEventListener('click', async () => {
  ...
  try {
    ...
    launchFill.style.width = '100%';
    overlay.classList.remove('active');
    actions.showWarningToast(`Modpack exported to: ${destPath.split(/[\\/]/).pop()}`);
  } catch (e) {
    overlay.classList.remove('active');
    ...
    hits.forEach(mod => {
      ...
      el.innerHTML = `
        ...
        <div class="mod-result-info">
          ...
          <div class="mod-result-meta"><span>Γ¼ç ${mod.downloads>=1000?(mod.downloads/1000).toFixed(0)+'K':mod.downloads}</span></div>
        </div>
        <button class="add-mod-btn ${installed?'installed':''}" ${installed?'disabled':''}>${installed?'Γ£ô Added':(state.browserMode==='modpack'?'+ Import':'+ Add')}</button>`;
...
async function mpAddItem(mod, btn, isDependency = false, passedMp = null) {
  ...
  if (state.browserMode === 'modpack' && !isDependency) {
    ...
    try {
      ...
      let modpackIcon = '';
      try {
        const modDetailsRes = await fetch(`https://api.curse.tools/v1/cf/mods/${projectId}`);
      ...
      if (!files.length) { hideImportOverlay(); actions.showWarningToast('No downloadable files found.'); if(btn){btn.textContent='+ Import';btn.disabled=false;} return; }
      ...
      if (!window.electronAPI) { hideImportOverlay(); actions.showWarningToast('Only available in the desktop app.'); return; }
      ...
      const loaderStr = rawLoaderId.toLowerCase();
      const loader = loaderStr.includes('fabric') ? 'Fabric' : loaderStr.includes('forge') ? 'Forge' : loaderStr.includes('neoforge') ? 'NeoForge' : 'Vanilla';
      // Extract pinned version: 'forge-14.23.5.2860' ΓåÆ '14.23.5.2860', 'fabric-0.15.11' ΓåÆ '0.15.11'
      const loaderVerMatch = rawLoaderId.match(/^[a-z]+-(.+)$/i);
      const loaderVersion = loaderVerMatch ? loaderVerMatch[1] : '';
      const mcVersion = manifest.minecraft?.version || '1.20.4';
      const newMp = { id: importRes.modpackId, name: manifest.name || modName, iconUrl: modpackIcon, mcVersion, loader, loaderVersion, mods: [], resourcepacks: [], shaders: [] };
      const mpData = JSON.parse(localStorage.getItem('idk_modpacks') || '[]');
      mpData.push(newMp);
      localStorage.setItem('idk_modpacks', JSON.stringify(mpData));
      state.modpacks.push(newMp);
      ...
      const downloadTask = async (f) => {
        try {
          ...
          completedCount++;
          const percent = manifestFiles.length ? 5 + (completedCount / manifestFiles.length) * 90 : 95;
          setImportProgress(`Downloading file ${completedCount} / ${manifestFiles.length}...`, percent);
        }
      };
  ...
  if (state.browserMode === 'modpack' && mod.provider === 'modrinth' && !isDependency) {
    if (btn) { btn.textContent = 'Fetching...'; btn.disabled = true; }
    showImportOverlay('Resolving modpack metadata...', 2);
    try {
      const projectId = typeof mod === 'string' ? mod : mod.project_id;
      ...
      const versionsRes = await fetch(`https://api.modrinth.com/v2/project/${projectId}/versions`);
      const versions = await versionsRes.json();
      if (!Array.isArray(versions) || !versions.length) { hideImportOverlay(); actions.showWarningToast('No versions found.'); if(btn){btn.textContent='+ Import';btn.disabled=false;} return; }
      
      // Get latest version
      const latestVersion = versions[0];
      const files = latestVersion.files || [];
      const primaryFile = files.find(f => f.primary) || files[0];
      if (!primaryFile) { hideImportOverlay(); actions.showWarningToast('No downloadable file found.'); if(btn){btn.textContent='+ Import';btn.disabled=false;} return; }
      ...
      const importRes = await window.electronAPI.downloadModrinthModpack({ downloadUrl: dlUrl });
      if (!importRes.success) throw new Error(importRes.error || 'Import failed');
      const manifest = importRes.manifest;
      const mcVersion = manifest.minecraft?.version || '1.20.4';
      const rawLoaderId = manifest.minecraft?.modLoaders?.[0]?.id || '';
  ...
  if (btn) { btn.textContent = 'Γ¼ç Fetching...'; btn.disabled = true; }
  try {
    ...
    if (provider === 'curseforge') {
      if (state.browserMode === 'mod') {
        if (mp.mods.find(m => m.modrinthId === projectId)) {
          if(btn) { btn.textContent = 'Γ£ô Added'; btn.classList.add('installed'); }
          return;
        }
        ...
        if (compatibleFile) {
          console.log(`[CurseForge] Γ£ô Found exact match: ${compatibleFile.fileName}`);
        } else {
        ...
        if (!compatibleFile) {
          if(btn) { actions.showWarningToast(`${modTitle} has no downloadable version`); btn.textContent='+ Add'; btn.disabled=false; }
          return;
        }
        ...
        if (btn) btn.textContent='Γ¼ç Installing...';
      ...
      if (state.browserMode === 'mod' || isDependency) {
        ...
        if (!versions.length) { 
          if(btn) { actions.showWarningToast(`${modTitle} has no version for MC ${mp.mcVersion} + ${mp.loader}`); btn.textContent='+ Add'; btn.disabled=false; }
          return; 
        }
        ...
        if (!res.ok) throw new Error(`Modrinth API error: ${res.status} ${res.statusText}`);
        versions = await res.json();
        if (!versions.length) { actions.showWarningToast(`${modTitle} has no version for MC ${mp.mcVersion}`); if(btn){btn.textContent='+ Add'; btn.disabled=false;} return; }
        const versionObj = versions[0];
        fileObj = versionObj.files.find(f=>f.primary)||versionObj.files[0];
        ...
        if (!res.ok) throw new Error(`Modrinth API error: ${res.status} ${res.statusText}`);
        versions = await res.json();
        if (!versions.length) { actions.showWarningToast(`${modTitle} has no downloadable version.`); if(btn){btn.textContent='+ Add'; btn.disabled=false;} return; }
        const versionObj = versions[0];
        fileObj = versionObj.files.find(f=>f.primary)||versionObj.files[0];
        entry = { modrinthId: projectId, name: modTitle, version: versionObj.version_number, filename: fileObj.filename, downloadUrl: fileObj.url, iconUrl: modIcon };
        mp.shaders.push(entry); mpSave(); 
    ...
    if (btn && !isDependency) {
      actions.showWarningToast(`Failed to add ${typeof mod === 'string' ? mod : mod.title}: ${e.message}`);
...
// --- Play Modpack ---
// IPC listeners are already registered globally above ΓÇö no setup needed here.
...
document.getElementById('btn-refresh-profiles').addEventListener('click', async () => {
  ...
  actions.modpacks = { mpGet, mpSave, mpRenderList, mpRenderDetail, loadProfilesFromDisk };
