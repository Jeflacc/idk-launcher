export function showListDialog({
  ...
  return new Promise((resolve) => {
    ...
    items.forEach((item) => {
      ...
      // Inline styles for the list button, to match the theme
      btn.style.cssText = `
        display: flex;
        align-items: center;
        width: 100%;
      ...
      if (item.icon) {
        btn.innerHTML = `<span style="margin-right: 12px; display: flex; align-items: center;">${item.icon}</span> <span>${item.label}</span>`;
      } else {
