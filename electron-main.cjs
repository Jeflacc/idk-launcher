/* --- MERGED START (ours + theirs) --- */`nconst { app, BrowserWindow, ipcMain, shell, dialog, globalShortcut, screen, protocol, net, nativeImage } = require('electron');
...
// Pull-based overlay data GÇö renderer requests this once it's ready
...
ipcMain.handle('remove-mod', async (event, { modpackId, filename }) => {
  ...
  try {
    ...
    // File not found GÇö treat as already removed (success)
...
// Auto Update GÇö electron-updater with GitHub provider
...
ipcMain.on('launch-modpack', async (event, args) => {
  ...
  // Read profile.json as fallback for missing values only GÇö trust renderer's values (from user settings) over disk.
    ...
    if (loaderNameLC === 'fabric') {
      ...
      if (existing) {
        ...
        try {
          ...
          // Network/API failure GÇö fall back to any cached Fabric loader on disk
...
function getRequiredJavaVersion(mcVersion, loader) {
  ...
  if (minor >= 999) return 25; // Non-1.x versions (e.g. 26.1.2) GåÆ Java 25

  if (minor >= 21) return 21; // 1.21+ GåÆ Java 21
  if (mcVersion === '1.20.5' || mcVersion === '1.20.6') return 21; // 1.20.5/1.20.6 require Java 21
  if (minor >= 17) return 17; // 1.17 to 1.20.4 GåÆ Java 17
  return 8; // 1.16.5 and below GåÆ Java 8
}
...
async function cleanCorruptFabricJars(dataPath) {
  ...
  // client jars renamed by MCLC, NOT the actual Fabric loader GÇö so only check ZIP
...
async function ensureVanillaClient(mcVersion, rootPath, progressCallback) {
  ...
  if (fs.existsSync(versionJsonPath)) {
    ...
    // Step 1 GÇö version manifest (only if we don't have the JSON yet)
...
async function installForge(mcVersion, rootPath, javaExe, progressCallback, pinnedVersion = null) {
  ...
  // 5. Run the Forge installer headlessly GÇö use spawn for real-time stderr capture
  ...
  if (!fs.existsSync(versionJson)) throw new Error('Forge installation failed GÇö version JSON not found after install.');
...
async function installNeoForge(mcVersion, rootPath, javaExe, progressCallback) {
  ...
  if (minor < 20 || (minor === 20 && patch <= 1)) {
    // NeoForge doesn't support 1.20.1 and below GÇö fall back to Forge
  ...
  if (!fs.existsSync(versionJson)) throw new Error('NeoForge installation failed GÇö version JSON not found after install.');
...
async function findExistingLoaderOnDisk(rootPath, mcVersion, loaderType) {
  ...
  for (const entry of entries) {
    ...
    let dirMcVersion;
    if (dirLoader === 'forge') {
      // e.g. "1.21.1-forge-47.1.0" GåÆ extract "1.21.1" before "-forge-"
      const fi = dirName.toLowerCase().indexOf('-forge-');
      if (fi > 0) dirMcVersion = dirName.substring(0, fi);
    } else if (dirLoader === 'neoforge') {
      // e.g. "neoforge-20.4.123" GÇö read JSON to find Minecraft version
      try {
      ...
      // e.g. "fabric-loader-0.16.9-1.21.1" GåÆ "-1.21.1" GåÆ "1.21.1"
...
function installFabric(version, rootPath, pinnedLoaderVersion = null) {
  return new Promise((resolve, reject) => {
    ...
    // because it would pin users to an outdated loader forever GÇö e.g. 0.19.2
...
ipcMain.handle('stop-frpc-tunnel', async () => {
  ...
  // Always return success GÇö if there's no active process, it's already stopped
  return { success: true };
});
...
ipcMain.handle('scan-profiles', async () => {
  try {
    ...
    function detectVersionFromMods(modsPath) {
      ...
      // just a prefix of the longer GÇö always prefer the more specific (longer) version.
      const versions = Object.keys(versionCounts);
    ...
    for (const entry of entries) {
      ...
      if (entry.name.startsWith('modpack-modpack-')) {
        ...
        // Only delete if it's empty (no mods) GÇö safety check
...
function getMinecraftDataPath() {
  ...
  // Fall back to default path GÇö never recurse
    ...
    https.get(url, { headers: { 'User-Agent': 'IDKLauncher/1.0 (contact@idklauncher.app)' } }, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          // Mods folder is inside the per-version profile GÇö no more cross-version leaking!
          const modsPath = path.join(profilePath, 'mods');
          if (!fs.existsSync(modsPath)) {
            fs.mkdirSync(modsPath, { recursive: true });
          } else {
